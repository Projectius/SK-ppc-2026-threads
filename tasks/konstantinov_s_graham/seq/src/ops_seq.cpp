...
void KonstantinovAGrahamSEQ::remove_duplicates(std::vector<double> &xs, std::vector<double> &ys) {
  ...
  pts.erase(std::unique(pts.begin(), pts.end(),
    ...
    return std::abs(a.first - b.first) < K_EPS && std::abs(a.second - b.second) < K_EPS;
  }),
            pts.end());
...
size_t KonstantinovAGrahamSEQ::find_anchor_index(const std::vector<double> &xs, const std::vector<double> &ys) {
  ...
  for (size_t i = 1; i < xs.size(); ++i) {
    if (ys[i] < ys[idx] - K_EPS || (std::abs(ys[i] - ys[idx]) < K_EPS && xs[i] < xs[idx] - K_EPS)) {
      idx = i;
    }
  }
  return idx;
...
double KonstantinovAGrahamSEQ::cross_val(const std::vector<double> &xs, const std::vector<double> &ys, size_t i,
                                         size_t j, size_t k) {
...
bool KonstantinovAGrahamSEQ::all_collinear_with_anchor(const std::vector<double> &xs, const std::vector<double> &ys,
...
std::vector<std::pair<double, double>> KonstantinovAGrahamSEQ::build_hull_from_sorted(
